...
async def handle_sessions_new(request: Request, websocket: WebSocket):
    """
    DEPRECATED: Handle sessions.new request.
    
    Session creation is now handled via the `/new` command.
    This maintains backwards compatibility by returning an error.
    """
    response = create_response(
        request_id=request.id,
        ok=False,
        error="sessions.new method is deprecated. Use the `/new` command instead."
    )
    await websocket.send_json(response)
...
async def handle_sessions_switch(
    ...
    """
    DEPRECATED: Handle sessions.switch request.
    
    Session switching is now handled via the `/switch <n>` command.
    This maintains backwards compatibility by returning an error.
    """
    response = create_response(
        request_id=request.id,
        ok=False,
        error="sessions.switch method is deprecated. Use the `/switch <number>` command instead."
    )
    await websocket.send_json(response)
    return
